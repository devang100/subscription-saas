...
export default function KanbanBoardPage() {
            ...
            days.push(
                <div key={d} className={`min-h-[120px] p-2 border border-gray-200 dark:border-zinc-800 relative group transition-colors hover:bg-white dark:hover:bg-zinc-900 ${isToday ? 'bg-indigo-50/30 dark:bg-indigo-900/10' : 'bg-gray-50/30 dark:bg-zinc-950'}`}>
                    ...
                    <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 cursor-pointer text-gray-300 hover:text-indigo-500 transition-opacity"
                        onClick={() => {
                            // Correct implementation for adding task on specific date
                            setEditingTaskId(null);
                            // Need to ensure timezone cleanliness, but simplistic approach for now
    ...
    useEffect(() => {
        ...
        const handleTaskCreate = (newTask: any) => {
            // Avoid duplicate additions if optimistic update happened (though here we don't optimistically create currently)
            setTasks(prev => {
        ...
        if (isOverColumn) {
            ...
            if (activeTask.status !== overColumnId) {
                setTasks(prev => {
                    ...
                    // Move to end of array to simulate append (simplified)
            ...
            if (activeTask.status !== overTask.status) {
                setTasks(prev => {
                    ...
                    // Logic to insert near the overTask
                    // We need to move activeTask to be close to overIndex in the flat list
                    // But "close" in flat list doesn't mean "visual neighbor" because of filter.
                    // However, if we preserve global relative order, filter preserves it.
                    return arrayMove(newTasks, activeIndex, overIndex);
                });
            } else {
                // Dragging within SAME column (Reordering)
                setTasks(prev => {
                    const activeIndex = prev.findIndex(t => t.id === activeId);
                    const overIndex = prev.findIndex(t => t.id === overId);
    ...
    const handleDragEnd = async (event: DragEndEvent) => {
        ...
        // Optimistic update is already done in DragOver for smoothness (mostly), 
        // but let's confirm logic.
        // Actually, pure sortable reordering inside simple State array is tricky if we don't sync.
        // We'll trust the final state matching visual.

        if (activeTask && newStatus && activeTask.status !== newStatus) {
            // Call API to persist status change
            try {
                // Ensure local state is firm
                setTasks(prev => {
                    const idx = prev.findIndex(t => t.id === active.id);
                    if (prev[idx].status === newStatus) return prev;
                    const newSt = [...prev];
                    newSt[idx] = { ...newSt[idx], status: newStatus };
                    return newSt;
                });
                await api.patch(`/tasks/${activeTask.id}`, { status: newStatus });
            } catch (error) {
